...
export default function Publish() {
  ...
  const sendable = (tier) =>
    inTier(tier).filter((c) => tier === 'A' || tier === 'C' || c.connected);
  ...
  async function send(tier) {
    const ids = sendable(tier).map((c) => c.id);
    ...
    try {
      ...
      while (job.status === 'running') {
        ...
        job = await api.get(`/publish/${jobId}`);
      }
      setResults((prev) => ({ ...prev, ...job.results }));
      speakOutcome(job.results);
    } catch (e) {
      setError(e.messageKey ?? 'error.unknown');
    } finally {
      setBusy(false);
    }
  }
  ...
  const attempted = (tier) => sendable(tier).length > 0 && sendable(tier).every((c) => results[c.id]);
  ...
  function row(c) {
    const state = RESULT_STATE[results[c.id]?.status] ?? pending(c);
    const goesToSetup =
