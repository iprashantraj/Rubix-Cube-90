...
import { Screen, BigButton, Card, YesNo, Spinner } from '../ui/kit.jsx';
import { IconRetry } from '../ui/icons.jsx';
 ...
 */
function displayable(url, fallback) {
...
export default function CatalogPrefill() {
  ...
  const [reject, setReject] = useState(null); // a message_key from the enhance gate
  ...
  if (step === 'enhancing') {
    return (
      <Screen prompt="enhance.working">
        <Card>
          <img src={draft.photoUrl} alt="" style={{ width: '100%', display: 'block', borderRadius: 14 }} />
        </Card>
        <Spinner />
      </Screen>
    );
  ...
  return (
    ...
    >
      <Card>
        <img src={image} alt="" style={{ width: '100%', display: 'block', borderRadius: 14 }} />
      </Card>
