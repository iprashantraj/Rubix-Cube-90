...
import type { GstRoute } from '../api/types';
import { useVoice } from '../voice/useVoice';
import { t } from '../i18n/index';
import { Screen, BigButton, Card, Chip, HelpButton } from '../ui/kit';
import { IconNext, IconYes, IconAlert } from '../ui/icons';
...
export default function GstWizard() {
  ...
  const { data: route, isPending, error } = useApiQuery<GstRoute>('/me/gst-route');
  const errKey = error ? ((error as ApiError).messageKey ?? 'error.unknown') : null;
  ...
  const meta = route ? ROUTES[route.route as keyof typeof ROUTES] : null;

  async function openPortal() {
    const url = PORTAL[route!.route as keyof typeof PORTAL];
    if (!url) return;
    try {
