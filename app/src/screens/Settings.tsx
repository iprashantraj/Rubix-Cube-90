...
import { record, transcribe, classifyYesNo } from '../voice/listen';
import { useVoice } from '../voice/useVoice';
import { LANGUAGES, t } from '../i18n/index';
import type { Lang } from '../i18n/index';
import { Screen, BigButton, Card, Chip, Grid, Tile, YesNo, Spinner } from '../ui/kit';
import { THEMES, DEFAULT_THEME } from '../ui/theme';
import {
  IconLanguage,
  IconMic,
  IconAlert,
...
export default function Settings() {
  ...
  const { data: me, error } = useApiQuery<Me>('/me');
        ...
        <Grid>
          {LANGUAGES.map((l) => (
            ...
            <Tile
              ...
              onClick={() => chooseLang(l.code as Lang)}
            />
          ))}
        </Grid>
      </Card>
      ...
      {me && (
        ...
        <Card>
          ...
          <p style={{ margin: 0, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {FLAGS.map((f) => (
              <Chip key={f} tone={me?.[f as keyof Me] ? 'done' : 'blocked'}>
                {t(lang, `onboard.${f}`)}
              </Chip>
            ))}
          </p>
