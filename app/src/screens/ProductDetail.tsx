...
export default function ProductDetail() {
  ...
  const { data: list, isPending: pendingP, error: errP } = useApiQuery<Product[]>('/products');
  const { data: channels, isPending: pendingC, error: errC } = useApiQuery<Channel[]>('/channels');
  // undefined = loaded and not found; null = still loading. Distinct on purpose:
  // 'product.not_found' and 'common.loading' are different sentences to speak.
  const product = list ? list.find((p) => p.id === id) : null;
  const isPending = pendingP || pendingC;
  const [results, setResults] = useState<Record<string, PublishResult> | null>(null); // channel id -> publish result, after a tap
  ...
  async function republish() {
    ...
    try {
      ...
      const job = (await api.post('/publish', {
        product_id: id,
        channels: oneTap.map((c) => c.id),
      })) as PublishJob;
...
function statusDot(status: PublishResult['status']) {
