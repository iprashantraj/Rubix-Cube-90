...
import { LANGUAGES, t, resolve } from '../i18n/index';
import type { Lang } from '../i18n/index';
import { speak, unlockAudio, isUnlocked, onUnlock } from '../voice/speak';
import { useSession } from '../store';
import { Screen, Grid, Tile } from '../ui/kit';
import { IconLanguage, IconReplay } from '../ui/icons';
import { ThreeScripts } from '../ui/illustrations';
...
function Begin({ onBegin }: { onBegin: () => void }) {
  const labels = [...new Set(LANGUAGES.map((l) => t((l.code as Lang), 'lang.begin')))];
  return (
...
export default function LangPick() {
  ...
  useEffect(() => {
    // onUnlock returns a boolean; an effect must return a cleanup or nothing.
    onUnlock(() => setReady(true));
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      for (const l of LANGUAGES) {
        ...
        const r = resolve((l.code as Lang), 'lang.title');
    ...
    <Screen>
      ...
      <Grid>
        {LANGUAGES.map((l) => (
          ...
          <Tile key={(l.code as Lang)} icon={IconLanguage} label={l.label} onClick={() => choose((l.code as Lang))} />
        ))}
      </Grid>
    </Screen>
  );
