...
function compose(
  ...
  // String(): the prefill comes from the vision model as loose JSON, so a field could be
  // a number or null. Coerced at the boundary rather than trusted — a listing field that
  // is not a string ends up rendered as "[object Object]" on a marketplace page.
  const str = (v: unknown, fallback = '') => (v == null ? fallback : String(v));
  return {
    title: str(prefill?.title, a.what ?? ''),
    desc: str(prefill?.desc_en, story),
    category: str(prefill?.category),
    material: str(prefill?.material, a.material ?? ''),
  };
}
...
export default function CatalogReview() {
  ...
  const [rec, setRec] = useState<RecHandle | null>(null);
  ...
  async function stopRec() {
    ...
    try {
      ...
      setLocal({ ...listing, [field as string]: transcript.trim() });
