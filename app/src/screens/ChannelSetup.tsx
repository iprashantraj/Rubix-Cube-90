...
import type { Channel, Me, SelectorPack } from '../api/types';
import { record, transcribe } from '../voice/listen';
import { useVoice } from '../voice/useVoice';
import { t } from '../i18n/index';
import { Screen, BigButton, Card, Chip, HelpButton } from '../ui/kit';
import { IconMic, IconNext, IconChannel, IconYes } from '../ui/icons';
...
const normalise = (s: string | null | undefined) => (s ?? '').toLowerCase().replace(/[^\p{L}]/gu, '');
...
async function openPortal(url: string) {
  ...
  try {
    ...
    await InAppBrowser.openInWebView({ url, options: { showURL: true, showToolbar: true } as never });
  } catch {
...
export default function ChannelSetup() {
  ...
  const { data: chans, error: ec } = useApiQuery<Channel[]>('/channels');
  const { data: me, error: em } = useApiQuery<Me>('/me');
  const { data: pack, error: ep } = useApiQuery<SelectorPack>(`/channels/${id}/selectorpack`);
  const channel = chans ? chans.find((c) => c.id === id) : null;
  const [mutError, setError] = useState<string | null>(null);
  const queryErr = ec ?? em ?? ep;
  const errKey = mutError ?? (queryErr ? ((queryErr as ApiError).messageKey ?? 'error.unknown') : null);
  ...
  const [verdict, setVerdict] = useState<'match' | 'mismatch' | 'unchecked' | null>(null); // null | 'match' | 'mismatch' | 'unchecked'
    ...
    <Screen prompt={prompt} promptVars={{ name: channel?.name ?? '' }} footer={<HelpButton />}>
      ...
      {tier && tier !== 'A' && !channel!.connected && !needsPrecheck && (
        <>
          ...
          {current && (
            <Card>
              <p style={{ margin: '0 0 8px' }}>{t(lang, current.voice_key ?? 'setup.title')}</p>
          ...
          */}
          {step === 0 && (
            <BigButton
              ...
              onClick={() => openPortal(PORTAL[id as keyof typeof PORTAL] ?? PORTAL.gem)}
            />
          )}
