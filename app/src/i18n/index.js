 ...
 * this one.
 */
const BUNDLES = { en: { ...en }, hi: { ...hi }, or: { ...or } };

for (const mod of Object.values(import.meta.glob('./strings/_new_*.json', { eager: true }))) {
  for (const [code, keys] of Object.entries(mod.default ?? mod)) {
    if (BUNDLES[code]) Object.assign(BUNDLES[code], keys);
  }
}
