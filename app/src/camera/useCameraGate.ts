...
function toGray(rgba: Uint8ClampedArray, out: Uint8Array): Uint8Array {
...
export function useCameraGate({
  ...
  const [problem, setProblem] = useState<string | null>('photo.too_dark');
  const [ready, setReady] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Refs, not state: these change every frame and must never trigger a re-render.
  const tiltRef = useRef<{ pitch: number; roll: number } | null>(tilt);
  tiltRef.current = tilt;
  const capturingRef = useRef(false);
  const onCaptureRef = useRef<typeof onCapture>(onCapture);
  onCaptureRef.current = onCapture;
  // Filled in by the scoring effect below so the manual shutter has something to call.
  const captureRef = useRef<null | (() => Promise<void>)>(null);
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices
      ...
      .catch((e: Error) =>
        setError(e.name === 'NotAllowedError' ? 'camera.denied' : 'camera.failed'),
      );
  ...
  useEffect(() => {
    ...
    const capture = async () => {
      ...
      try {
        ...
        const blob = await new Promise<Blob>((res) =>
          best.canvas.toBlob((b) => res(b as Blob), 'image/jpeg', 0.92),
        );
    ...
    const loop = () => {
      ...
      const metrics = analyse(gray, GATE_W, GATE_H, thresholds, tiltRef.current as never);
