...
colors:
  ...
  tile-strong: "#7a2e1a"
  coral: "#9c3d24"
  ok: "#1f9d55"          # the app's own green — the gate turning green, a channel going live. Never decorative.
  navy: "#151816"
  navy-soft: "#1B1F1D"
  navy-elev: "#202522"
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — an EB Garamond figure paired with a JetBrains Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (EB Garamond italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. system-ui italic, small system-ui uppercase cite beneath."
  section-rule:
...
- **Fonts** — already set to **system-ui** (display) / **Noto Sans Devanagari** (body); ignore any preset font name lingering in prose.
...
Three editorial voices, each in its own face: **system-ui** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **system-ui** carries body, leads, card titles, buttons, and
...
- **system-ui** (sentence case, negative-tracked) for all display; **system-ui** body/chrome; **JetBrains Mono** index + code.
...
- **Squint** — one system-ui display moment dominates at 3–6× its neighbor.
...
- **Type** — system-ui sentence-case display (negative-tracked); system-ui 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (system-ui `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (system-ui `headline` 4.6cqw
...
- **system-ui display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **system-ui body** sentence case weight 400. **JetBrains Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — system-ui figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — system-ui italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized system-ui · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line system-ui `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** system-ui 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single system-ui line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line system-ui `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, number-lockup, lead/caption, optional `section-rule`. **Focal** a system-ui `number-hero` figure with a mono unit over a 1px rule — the PR impact (`+1,204 / −318`, `17 files`, `2.1× faster`). **Chrome** mono tag. **Accent** the figure in ink; at most one coral unit. **Fixed** serif figure + mono unit, hairline rule. **Free** the figures (from the script), tag, layout + density. **Density** free.

### 5 · Pull-quote (quote · move: system-ui italic · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, pull-quote, tag-upper cite. **Focal** a system-ui **italic** `quote-pull` — a commit message, a reviewer line, or the thesis — with a small system-ui uppercase cite (author · role) beneath. **Chrome** mono kicker. **Accent** none, or one coral mark. **Fixed** system-ui italic quote + uppercase cite. **Free** quote, attribution, layout + density. **Density** free.

### 6 · Closing / CTA (closer · move: coral voltage · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}`). **Composes** display sign-off, coral-callout, optional contributor row (`assets/<login>.png` avatars + mono names). **Focal** a short system-ui sign-off with the one **coral-callout** (the CTA or full-bleed band) and, for a "shipped-by" close, a row of hairline-ringed avatar chips. **Chrome** mono index. **Accent** the single coral voltage. **Fixed** one coral moment, hairline avatar rings, sentence-case serif. **Free** sign-off, who ships, layout + density. **Density** free.
...
- Set all display in **system-ui, sentence case**, negative-tracked; **system-ui 400** body; **JetBrains Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair an EB Garamond figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case system-ui display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one system-ui display moment dominates.
...
- **Type** — system-ui sentence-case display (negative-tracked); system-ui 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
